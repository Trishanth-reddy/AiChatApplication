...
function Memory() {
  // CRITICAL: Never expose your API key in the frontend. Move this to a .env file
  const apiKey = "YOUR_MISTRAL_API_KEY_HERE";
  const client = new Mistral({ apiKey: apiKey });
  
  const { memoryData, setMemoryData } = useContext(MemoryContext);
  const { wyd, know, trait, pdfFile, structuredData } = memoryData;
  ...
  const handlePdfChange = async (e) => {
    ...
    setIsLoading(true);
    setMemoryData(prev => ({ ...prev, pdfFile: file, pdfText: "Extracting text...", structuredData: null }));

    try {
      const text = await pdfToText(file);
      setMemoryData(prev => ({ ...prev, pdfFile: file, pdfText: text }));

      const chatResponse = await client.chat.complete({
        model: "mistral-small-latest",
        messages: [{
          role: "user",
          content: `Analyze the following resume text and convert it into a structured JSON object. The JSON must include keys like contactInfo, summary, technicalSkills, experience, projects, and education. Ensure the output is only the raw JSON object and nothing else.
...
---`
      ...
      if (jsonStartIndex !== -1 && jsonEndIndex !== -1) {
        ...
        try {
          ...
          setMemoryData(prev => ({ ...prev, structuredData: parsedData }));
        } catch (parseError) {
          setMemoryData(prev => ({ ...prev, structuredData: { error: "Failed to parse JSON from API response." } }));
        }
      } else {
        setMemoryData(prev => ({ ...prev, structuredData: { error: "No JSON object found in API response." } }));
      }
    } catch (error) {
      console.error("Failed to process PDF or call API:", error);
      setMemoryData(prev => ({ ...prev, pdfText: "Error: Could not extract or process text from this PDF.", structuredData: null }));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleClearPdf = () => {
    setMemoryData(prev => ({ ...prev, pdfFile: null, pdfText: "", structuredData: null }));
  };
        ...
        <div className="flex flex-col max-h-[95vh] w-full max-w-7xl p-6 sm:p-8 rounded-2xl bg-white/5 backdrop-blur-2xl shadow-2xl border border-white/10 overflow-y-auto scrollbar-thin scrollbar-thumb-white/20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 flex-grow">
            ...
            <div className="bg-white/5 backdrop-blur-xl rounded-2xl p-6 border border-white/10 flex flex-col">
              ...
              <div className="flex-grow">
                  ...
                  <div className="mt-4 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        ...
                        <span className="text-emerald-300 text-sm font-medium truncate">{pdfFile.name}</span>
                      </div>
                      <button
                ...
                {structuredData && (
                  <div className="mt-6 p-4 bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 max-h-64 overflow-y-auto">
          ...
          <div className="flex justify-end mt-8 pt-6 border-t border-white/10">
            <button
              ...
              className="flex items-center space-x-2 px-6 py-3 bg-gradient-to-r from-violet-500 to-purple-500 hover:from-purple-500 hover:to-violet-500 rounded-xl font-semibold transition-all duration-200 hover:scale-105 hover:shadow-lg hover:shadow-violet-500/25"
            >
